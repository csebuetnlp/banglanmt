...
def score_with_trans(x, y, xTrans, fwd_mean, bwd_mean, xTrans_fwd_mean, xTrans_bwd_mean, margin, verbose=False):
    return margin(x.dot(y), (fwd_mean + bwd_mean) / 2, xTrans.dot(y), (xTrans_fwd_mean + xTrans_bwd_mean) / 2)
...
def score_candidates_with_trans(x, y, xTrans, candidate_inds, fwd_mean, bwd_mean, xTrans_fwd_mean, xTrans_bwd_mean, margin, verbose=False):
    if verbose:
        print(' - scoring {:d} candidates'.format(x.shape[0]))
    scores = np.zeros(candidate_inds.shape)
    for i in range(scores.shape[0]):
        for j in range(scores.shape[1]):
            k = candidate_inds[i, j]
            scores[i, j] = score_with_trans(x[i], y[k], xTrans[i], fwd_mean[i], bwd_mean[k], xTrans_fwd_mean[i], xTrans_bwd_mean[k], margin)
    return scores
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trans', action='store_true',
        help='Use translations for scoring')
    ...
    if args.trans:
        srcTransFile = ".enTranslated".join(args.src.rsplit('.bn', 1))
        srcTrans_inds, srcTrans_sents = TextLoadUnify(srcTransFile, args)
    ...
    if args.trans:
        xTrans = EmbedLoad(".enTranslated".join(args.src_embeddings.rsplit('.bn', 1)), args.dim, verbose=args.verbose)

    if args.unify:
        x = unique_embeddings(x, src_inds, args.verbose)
        if args.trans:
            xTrans = unique_embeddings(xTrans, src_inds, args.verbose)
    
    if args.trans:
        faiss.normalize_L2(xTrans)
    ...
    if args.retrieval is not 'bwd':
        ...
        if args.trans:
            xTrans2y_sim, xTrans2y_ind = knn(xTrans, y, min(y.shape[0], args.neighborhood), args.gpu)
            xTrans2y_mean = xTrans2y_sim.mean(axis=1)
    ...
    if args.retrieval is not 'fwd':
        ...
        if args.trans:
            y2xTrans_sim, y2xTrans_ind = knn(y, xTrans, min(xTrans.shape[0], args.neighborhood), args.gpu)
            y2xTrans_mean = y2xTrans_sim.mean(axis=1)

    # margin function
    if args.trans:
        if args.margin == 'absolute':
            margin = lambda a, b, c, d: c / d
        elif args.margin == 'distance':
            margin = lambda a, b, c, d: 2 / (1 / (a - b) + 1 / (c - d))
    ...
    if args.mode == 'search':
        ...
        if args.trans:
            fwd_scores = score_candidates_with_trans(x, y, xTrans, x2y_ind, x2y_mean, y2x_mean, xTrans2y_mean, y2xTrans_mean, margin, args.verbose)
            bwd_scores = score_candidates_with_trans(y, x, xTrans, y2x_ind, y2x_mean, x2y_mean, y2xTrans_mean, xTrans2y_mean, margin, args.verbose)
        else:
