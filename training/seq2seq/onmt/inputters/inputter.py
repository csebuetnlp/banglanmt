...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
                 ...
                 repeat=True, num_batches_multiple=1, yield_raw_example=False, swap_sides=False):
    ...
    def _iter_dataset(self, path):
        ...
        if self.swap_sides:
            logger.info('Swapping source and target sentences with 0.5 prob')

            for i, example in enumerate(cur_dataset.examples):
                if random.random() > 0.5:
                    tmp = example.src
                    cur_dataset.examples[i].src = cur_dataset.examples[i].tgt
                    cur_dataset.examples[i].tgt = tmp

            # print("*****************Src Lines:(first 5)************")
            # for i in range(5):
            #     print(cur_dataset.examples[i].src)

            # print("*****************Tgt Lines:(first 5)************")
            # for i in range(5):
            #     print(cur_dataset.examples[i].tgt)
